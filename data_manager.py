...
SHEETY_USERS_ENDPOINT = os.getenv("BASE_URL_SHEETY") + "/" + os.getenv("USERNAME_SHEETY") + "/" + os.getenv("PROJECT_NAME_SHEETY") + "/" + os.getenv("SECOND_SHEET_NAME_SHEETY") # get the url of the sheet
...
class DataManager:
    ...
    def get_customer_emails(self):
        ...
        """
        response = requests.get(url=f"{SHEETY_USERS_ENDPOINT}", headers=self._authorization) # get the data from the sheet
        data = response.json()
